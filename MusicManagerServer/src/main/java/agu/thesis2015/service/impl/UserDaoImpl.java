...
import agu.thesis2015.service.UserDao;
...
public class UserDaoImpl implements UserDao {
	...
	public String userProcessor(String context) {
		...
		case GET: {
			...
			} else if (message.getAction().equals(MessageAction.ACTIVE)) {
				response = getUserWithPass(request.getUsername()).toJson();
			}
			break;
		}
		case POST: {
		...
		case PUT: {
			if (message.getAction().equals(MessageAction.UPDATE)) {
				...
				response = activeUser(request.getListId().get(0)).toJson();
			}
			break;
		}
		case DELETE: {
	...
	public Response activeUser(String username) {
		try {
			...
			user.setActive(true);
			userRepo.save(user);
			return getResponse(ResponseStatus.OK, 200, "Active success!", null);
		} catch (Exception e) {
			return getResponse(ResponseStatus.INTERNAL_SERVER_ERROR, 500, e.getMessage(), null);
		}
	}
		...
		try {
			...
			if (user != null) {
				if (user.isActive())
					...
					return getResponse(ResponseStatus.BAD_REQUEST, 400, "User isn't active", null);
			} else
				return getResponse(ResponseStatus.BAD_REQUEST, 400, "User isn't exsist", null);
		} catch (Exception e) {
			return getResponse(ResponseStatus.INTERNAL_SERVER_ERROR, 500, e.getMessage(), null);
